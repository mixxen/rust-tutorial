...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="rust-lesson-baseline-") as directory:
        practice = Path(directory) / "practice"
        practice.mkdir()
        ...
        command = ["cargo", "test", "--locked", "--offline", "--test", "acceptance", "--", "--color", "never"]
        result = subprocess.run(command, cwd=practice, capture_output=True, text=True, check=False, timeout=120)
        ...
        print("PASS: untouched strict-rule baseline fails exactly the four intended checks")
        shutil.copy2(ROOT / "solutions/00-rust-orientation/src/lib.rs", practice / "src/lib.rs")
        subprocess.run(command, cwd=practice, check=True, timeout=120)
